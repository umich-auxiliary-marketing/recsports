...
class FrmProEntriesController{
	...
	public static function process_update_entry( $params, $errors, $form, $args ) {
		self::maybe_autosave_on_page_turn( $params, $errors, $form );
	 ...
	 * @since 2.3
	 */
	private static function maybe_autosave_on_page_turn( $params, $errors, $form ) {
		if ( ! empty( $errors ) ) {
			return;
		}
		...
		if ( $drafts_allowed && is_user_logged_in() ) {
			self::autosave_on_page_turn( $params, $form );
		}
	}
	 ...
	 * @since 2.3
	 */
	private static function autosave_on_page_turn( $params, $form ) {
		...
		if ( empty( $errors ) ) {
			if ( $is_ajax_on ) {
				...
				if ( ( ! isset( $_POST[ 'frm_page_order_' . $form->id ] ) && ! $going_backwards ) || FrmProFormsHelper::saving_draft() ) {
					...
					self::maybe_autosave_on_page_turn( $_POST, $errors, $form );
