...
class FrmProEntryMeta{
    ...
    public static function validate($errors, $field, $value, $args) {
		...
		self::validate_time_field( $errors, $field, $value );
    ...
    public static function validate_unique_field(&$errors, $field, $value) {
        ...
        if ( $field->type == 'time' ) {
            if ( self::is_datetime_used( $field, $value, $entry_id ) ) {
	...
	private static function get_entry_ids_for_date( $values ) {
		...
		self::add_field_to_query( $values['date_key'], $query );
	...
	private static function get_used_times_for_entries( $values ) {
		...
		self::add_field_to_query( $values['time_key'], $query );
	...
	private static function validate_time_field( &$errors, $field, $value ) {
		if ( $field->type != 'time' ) {
			return;
		}
